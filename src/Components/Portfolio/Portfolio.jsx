...
const Card = ({ image, title, description, onClickRoutes, date, isSmallScreen }) => {
  ...
  return (
    ...
    >
      ...
      <div className="p-4">
        <h2 className="text-lg text-heading font-semibold underline" onClick={() => { onClickRoutes(`Case study - ${title}`) }}>{title}</h2>
      ...
      {isHovered && !isSmallScreen && (
        <div className="absolute inset-0 flex items-center justify-center">
          ...
          <button onClick={() => { onClickRoutes(`Case study - ${title}`) }} className="relative text-sm text-heading flex items-center bg-[rgb(var(--color-sidebar))] hover:bg-opacity-80 focus:ring-2 focus:ring-white rounded-lg px-4 py-2.5 transition-all duration-300">
